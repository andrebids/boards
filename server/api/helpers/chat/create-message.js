...
module.exports = {
  ...
  async fn(inputs) {
    ...
    const message = await sails.getDatastore().transaction(async (db) => {
      ...
      const access = await assertConversationWritable(inputs.conversation.id, inputs.user.id, db);
      uniqueRecipientUserIds =
        inputs.conversation.type === ChatConversation.Types.PROJECT_GROUP
          ? access.memberUserIds
          : access.participants.map(({ userId }) => userId);
    ...
    return withConversationLock(inputs.conversation.id, async () => {
      uniqueRecipientUserIds = await sails.helpers.chat.getConversationRecipientUserIds(
        inputs.conversation,
      );
