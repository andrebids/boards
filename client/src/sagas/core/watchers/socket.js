...
const createSocketEventsChannel = () =>
  eventChannel((emit) => {
    ...
    const handleUserPresenceUpdate = ({ item: { presences } }) => {
      emit(entryActions.handleUserPresenceUpdate(presences));
    };
    ...
    const handleChatMessageAttachmentCreate = api.makeHandleChatMessageAttachmentCreate(
      ({ messageId, item }) => {
        emit(entryActions.handleChatMessageAttachmentCreate(messageId, item));
      },
    );
    ...
    socket.on('userPresenceUpdate', handleUserPresenceUpdate);
    ...
    return () => {
      ...
      socket.off('userPresenceUpdate', handleUserPresenceUpdate);
