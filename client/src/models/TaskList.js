...
export default class extends BaseModel {
  ...
  duplicate(id, data, rootId) {
    ...
    this.tasks.toModelArray().forEach((taskModel) => {
      nextTaskIdByTaskId[taskModel.id] = `${taskModel.id}-${rootId}`;
    });

    this.tasks.toModelArray().forEach((taskModel) => {
      taskModel.duplicate(nextTaskIdByTaskId[taskModel.id], {
        taskListId: taskListModel.id,
        parentTaskId: taskModel.parentTaskId ? nextTaskIdByTaskId[taskModel.parentTaskId] : null,
      });
    });
